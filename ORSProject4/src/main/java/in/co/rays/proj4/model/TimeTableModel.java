	...
	public List Search(TimeTableBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getExam_Date() != null && bean.getExam_Date().getTime() > 0) {
				Date d =  new java.sql.Date(bean.getExam_Date().getTime());
				System.out.println("done");
				sql.append(" AND Exam_Date = '" + DataUtility.getDateString(d)+ "'");
			}
			if (bean.getExam_Time() != null && bean.getExam_Time().length() > 0) {
				/* Date d = new Date(bean.getExam_Date().getTime()); */
				System.out.println("done");
				sql.append(" AND Exam_Time = '" + bean.getExam_Time() + "'");
				System.out.println("nice");
			}

			/*
			 * if (bean.getExam_Date() != null) { Date d = new
			 * Date(bean.getExam_Date().getTime()); System.out.println("done");
			 * sql.append(" AND Exam_Date = '" + bean.getExam_Date() + "'");
			 * System.out.println("nice");
			 */
